document.addEventListener('DOMContentLoaded', function() {
    ...
    capacitacionForm.addEventListener('submit', function(e) {
        ...
        // Crear un objeto FormData a partir del formulario para enviar los datos
            ...
            .then(data => {
                ...
                if (data.status === 'success') {
                    Swal.fire({
                        ...
                        // Cerrar el modal de agregar capacitación
                        $('#addCapacitacionModal').modal('hide');

                        // Recargar los eventos en el calendario para reflejar los cambios
                        calendar.refetchEvents();

                        // Recargar el slider de flyers si la función cargarFlyers está definida
                        if (typeof cargarFlyers === "function") {
                            cargarFlyers();
                        }

                        // Actualizar la leyenda del mes actual en el calendario
                        const currentDate = calendar.getDate();
                        const mes = currentDate.getMonth() + 1; // Mes actual (1-indexado)
                        const anio = currentDate.getFullYear();
                        actualizarLeyenda(mes, anio); // Actualizar la leyenda
                    });
                } else {
